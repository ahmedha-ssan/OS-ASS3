...
public class BestFit {
    ...
    public BestFit(ArrayList<Partition> p, ArrayList<Process> p2) {
        AllPartitions = p;
        AllProcesses = p2;
    ...
    public void addBestFit() {
        for (int i = 0; i < AllProcesses.size(); i++) {
            ...
            for (int j = 0; j < AllPartitions.size(); j++) {
                if(Min>AllPartitions.get(j).size&&AllPartitions.get(j).processName == " "&&AllPartitions.get(j).size >= AllProcesses.get(i).size){
                    Min=AllPartitions.get(j).size;
